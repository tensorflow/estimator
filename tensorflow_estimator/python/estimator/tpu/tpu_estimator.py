...
_tpu_estimator_gauge = tf.__internal__.monitoring.BoolGauge(
...
class TPUInfeedOutfeedSessionHook(tf.compat.v1.train.SessionRunHook):
  ...
  def begin(self):
    ...
    for op in summary_writer_init_ops:
      self._finalize_ops.append(tf.summary.flush(writer=op.inputs[0]))
...
class _Inputs(object):
  ...
  def from_input_fn(return_values):
    ...
    if isinstance(return_values, tf.data.Dataset):
