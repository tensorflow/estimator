...
class KerasPremadeModelTest(tf.test.TestCase):
  ...
  def test_train_premade_linear_model_with_dense_features(self):
    ...
    model = tf.keras.Model(inputs=keras_input, outputs=h)
  ...
  def test_train_premade_widedeep_model_with_feature_layers(self):
    ...
    combined_linear = tf.keras.Sequential([linear_feature_layer, linear_model])
    ...
    combined_dnn = tf.keras.Sequential([dnn_feature_layer, dense_layer])
