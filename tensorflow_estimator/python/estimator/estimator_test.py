...
class EstimatorExportTest(test.TestCase):
  ...
  def test_export_all_saved_models_metric_operation(self):
    ...
    def _model_fn(features, labels, mode):
      del features, labels  # Unused
      metrics = {
          'metrics': (constant_op.constant([0]), control_flow_ops.no_op())
      }
      return model_fn_lib.EstimatorSpec(
          mode,
          predictions=constant_op.constant(10.),
    ...
    export_dir = est._export_all_saved_models(export_dir_base,
                                              input_receiver_fn_map)
    ...
    with ops.Graph().as_default() as graph:
      with session.Session(graph=graph) as sess:
        ...
        self.assertEqual(sig_outputs['metrics/update_op'].name,
                         'metric_op_wrapper:0')
