...
class ARModel(model.TimeSeriesModel):
  ...
  def _process_exogenous_features(self, times, features):
    ...
    if embedded is None:
      ...
      return tf.zeros(
          tf.concat([tf.compat.v1.shape(times), tf.constant([0])],
                           axis=0))
    else:
      return embedded
  ...
  def predict(self, features):
    ...
    predict_times = tf.cast(
        ops.convert_to_tensor(features[PredictionFeatures.TIMES]), tf.dtypes.int32)
    ...
    with tf.control_dependencies(
        [tf.compat.v1.debugging.assert_equal(tf.compat.v1.shape(predict_times)[1],
    ...
    padding_size = (prediction_iterations * self.output_window_size -
                    num_predict_values)
    predict_times = tf.compat.v1.pad(
        predict_times, [[0, 0], [0, padding_size]])
    exogenous_regressors = tf.compat.v1.pad(
    ...
    state_times = tf.cast(
        ops.convert_to_tensor(state_times), tf.dtypes.int32)
    ...
    if self.input_window_size > 0:
      ...
      with tf.control_dependencies([
          tf.compat.v1.debugging.assert_greater_equal(values_size, self.input_window_size),
  ...
  def _process_window(self, features, mode, exogenous_regressors):
    ...
    prediction = prediction_ops["mean"]
    covariance = prediction_ops["covariance"]
    targets = tf.slice(values, [0, self.input_window_size, 0],
                              [-1, -1, -1])
  ...
  def get_batch_loss(self, features, mode, state):
    ...
    if mode == estimator_lib.ModeKeys.TRAIN:
      ...
      num_iterations = ((tf.compat.v1.shape(times)[1] -  self.input_window_size)
                        // self.output_window_size)
      ...
      _, loss_ta, mean_ta, covariance_ta = tf.compat.v1.while_loop(
          ...
          [0,
           ...
           tf.TensorArray(dtype=self.dtype, size=num_iterations),
           tf.TensorArray(dtype=self.dtype, size=num_iterations)])
      values = tf.cast(features[TrainEvalFeatures.VALUES],
                             dtype=self.dtype)
      batch_size = tf.compat.v1.shape(times)[0]
      ...
      return model.ModelOutputs(
          ...
          predictions={
              ...
              "covariance": tf.reshape(
                  tf.compat.v1.transpose(covariance_ta.stack(), [1, 0, 2, 3]),
                  prediction_shape),
  ...
  def _compute_time_features(self, time):
    ...
    mod = (tf.cast(time % periods, self.dtype) * self._buckets /
    ...
    intervals = tf.reshape(
        tf.range(self._buckets, dtype=self.dtype),
        [1, 1, 1, self._buckets])
