...
class LinearSDCA(object):
  ...
  def get_train_step(self, state_manager, weight_column_name, loss_type,
    ...
    cache = tf.__internal__.feature_column.FeatureTransformationCache(features)
    ...
    for column in sorted(feature_columns, key=lambda x: x.name):
      ...
      elif isinstance(column, tf.__internal__.feature_column.DenseColumn):
...
def _get_expanded_variable_list(var_list):
  ...
  for variable in var_list:
    if (isinstance(variable, tf.Variable) or
        tf.__internal__.ops.is_resource_variable(variable) or
...
class _LinearModelLayer(tf.keras.layers.Layer):
  ...
  def __init__(self,
    ...
    for column in self._feature_columns:
      if not isinstance(column, (tf.__internal__.feature_column.DenseColumn,
                                 fc_v2.CategoricalColumn)):
        raise ValueError(
